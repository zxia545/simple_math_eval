...
if __name__ == "__main__":
    ...
    if args.model_path:
        ...
        if process_id is None:
            ...
            print(f"vLLM server started with process ID: {process_id}")
            # if , in input_file, then it is a list of jsonl files
            if ',' in args.input_file:
                input_files = args.input_file.split(',')
                output_files = args.output_file.split(',')
                for i in range(len(input_files)):
                    gen_math(input_files[i], output_files[i], args.api_base, args.model_name, args.max_tokens, args.temperature, args.threads)
            else:
